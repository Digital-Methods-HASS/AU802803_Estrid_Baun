...
#library(tidyverse)
kings1 <- read.csv("data/kingsEstrid.csv")

kings2 <- read_csv("data/kingsEstrid.csv")

kings3 <- read.csv2("data/kingsEstrid.csv")

kings4 <- read_csv2( "data/kingsEstrid.csv")
```
Answer: 
1. Which of these functions is a `tidyverse` function? Read data with it below into a `kings` object
    Because I use a Danish computer, the data in the dataset is seperated by semicolons. This means that       csv2 is the argument to use. Therefore csv2 is a tidyverse function on my Danish computersystem.
    Likewise when using tiyverse, one must use uncerscore. The period is the R-studio basis, but when          installing tidyverse, use underscore
    The golden combo is _csv2
    
2. What is the result of running `class()` on the `kings` object created with a tidyverse function.
    It shows the class of the dataset named kings as being "tbl" which means tibble, which behaves as a        dataframe

3. How many columns does the object have when created with these different functions?
    In my specific data set there are 6 columns which is showed by the argument ncol(kings)
...
kings <- read_csv2( "data/kingsEstrid.csv")
...
view(kings) #could also use commands like summarize or glimpse
...
You can calculate the duration of reign in years with `mutate` function by subtracting the equivalents of your `startReign` from `endReign` columns and writing the result to a new column called `duration`. But first you need to check a few things:
    I have already done so in advance in my dataset. The column "Regeringstid" is what i made in advance       when originally making the dataset, but to do it, use the mutate command and name the column "duration"     as shown before
...
kings<-kings %>% 
  mutate(
    DanskKongeFra=as.numeric(DanskKongeFra),
    DanskKongeTil=as.numeric(DanskKongeTil),
    duration=DanskKongeTil-DanskKongeFra) %>%
  mutate(midyear=DanskKongeFra+duration/2)
```
...
#The average turns out to be 20.5 years
```
...
kings_longest_rulers<-kings %>% 
  ...
  slice(1:3) %>% 
  mutate(Days=duration*365+(duration%/%4)) #this adds leap years
...
ggplot(kings,aes(x=midyear,y=duration))+
  ...
  geom_smooth(method="loess",se=TRUE,color="pink")+
  labs(title="Duration of Reign of the Danish Royal Lineage",
       x="Midyear of Reign",
