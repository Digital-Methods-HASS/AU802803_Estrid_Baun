...
names(Western_Europe)[4] <- "Homicide Rates"
...
unique(Western_Europe$Countries)
  
Western_Europe %>% 
  ggplot(aes(x=Year,y=`Homicide Rates`,color=Countries))+
  geom_line()+
  theme_classic()+
...
ggsave("homicides1.png", width =14, height =6, dpi= 100)
...
```{r eval = TRUE}
# YOUR CODE HERE:
read_csv2("data/kingsEstrid.csv")
kings <- read_csv2("data/kingsEstrid.csv")

kings %>% 
  mutate(duration = DanskKongeTil-DanskKongeFra) %>% 
  mutate(mid_year= DanskKongeTil + duration/2) %>% 
  ggplot(aes(x = mid_year, y = duration)) +
  geom_point(color = "violet")+
  geom_smooth(color="darkred")+
  labs(title ="Danish Royal Lineage")+
    theme_bw() +
  theme(text = element_text(size = 16))
```
...
By using the two plots, "Historical homicides in Western Europe" and "Danish Monarchs", we can reflect on if we have become more civilized over time.

In the "Historical homicides in Western Europe" plot, the trend shows that homicide decrease over time. This suggests that violence has become less common, which is often associated with increased social stability, stronger institutions, and improved legal systems.

In the "Danish Monarchs" plot, the trend indicates that the duration of kings reigns increases over time. Earlier monarchs tend to have shorter reigns, while more recent monarchs rule for longer periods. One possible explanation is that in earlier periods there were more unstable and violets, increasing the risk of being killed in a homicide.
As homicide rates decline, the duration of kings’ reigns increases. This suggests a development toward more stable and less violent societies.

The two plots can show that societies have become more civilized over time, because of decreasing violence and increasing stability.